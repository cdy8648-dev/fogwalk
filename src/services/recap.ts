...
export interface RecapData {
  since: number;
  until: number;
  /** 별자리 점(시간순) — 외출 단위로 분리된 세그먼트. 세그먼트 사이엔 선을 긋지 않는다. */
  segments: RecapPoint[][];
  tileCount: number;
  km: number;
  landmarks: Landmark[]; // 기간 내 발견(등급 높은 순, 상한 12)
  regionName: string | null; // '서울'·'충남' 등 (해외는 국가명)
...
function buildRecap(): RecapData | null {
  ...
  // 외출 단위 세그먼트 분리 — 거리/시간이 크게 벌어진 지점에서 선을 끊는다.
  // (서로 다른 외출을 한 선으로 이어 스파게티가 되던 문제)
  const segments: RecapPoint[][] = [];
  let seg: RecapPoint[] = [points[0]];
  for (let i = 1; i < points.length; i++) {
    const gapM = haversineMeters(points[i - 1], points[i]);
    const gapMs = points[i].ts - points[i - 1].ts;
    if (gapM > CONFIG.RECAP_GAP_SPLIT_M || gapMs > CONFIG.RECAP_GAP_SPLIT_MS) {
      segments.push(seg);
      seg = [];
    }
    seg.push(points[i]);
  ...
  // 지역 타이틀 + 달성률: 폴백 = 도청 최근접 (checkRecap이 역지오코딩으로 정밀 보정)
  ...
  return {
    ...
    regionPct: region ? regionPctText(region) : null,
  };
}

/** KR 시/도 달성률 문자열 (면적 미상이면 null). */
function regionPctText(key: string): string | null {
  const area = regionAreaKm2('KR', key);
  if (!area) return null;
  const regionTiles = getRegionStats('KR')
    .filter((r) => regionKey(r.region) === key)
    .reduce((s, r) => s + r.tiles, 0);
  const pct = ((regionTiles * TILE_AREA_KM2) / area) * 100;
  return pct >= 10 ? `${Math.round(pct)}%` : pct < 0.05 ? '<0.1%' : `${pct.toFixed(1)}%`;
}
 ...
 * 지역 타이틀은 역지오코딩 1회로 정밀 보정(포그라운드 전용이라 배터리 헌장과 무관).
 * 도청 최근접 추정은 경계 지역에서 옆 시/도로 새는 문제가 있었음(충남→충북 오표기).
 */
export async function checkRecap(): Promise<void> {
  const store = useRecapStore.getState();
  if (store.playing) return; // 재생 중엔 재계산하지 않음
  const data = buildRecap();
  if (!data) return;

  // 정밀 지역명: 별자리 중간 지점을 역지오코딩 (실패 시 폴백값 유지)
  try {
    const flat = data.segments.flat();
    const mid = flat[Math.floor(flat.length / 2)];
    const c = await detectCountry(mid.lat, mid.lng);
    if (c?.code === 'KR' && c.region) {
      const key = regionKey(c.region);
      if (key) {
        data.regionName = key;
        data.regionEn = regionEnName('KR', key);
        data.regionPct = regionPctText(key);
      }
    } else if (c && c.code !== 'KR') {
      data.regionName = c.name; // 해외: 국가명 (예: '태국의 별자리')
      data.regionEn = c.code;
      data.regionPct = null;
    }
  } catch {
    /* 역지오코딩 실패 — 최근접 추정 폴백 유지 */
  }
